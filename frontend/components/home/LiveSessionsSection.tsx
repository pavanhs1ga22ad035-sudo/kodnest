      ...
      <div className="flex gap-4 overflow-x-auto pb-2">
        {liveSessions.map((session) => (
          <article key={session.id} className="min-w-[300px] max-w-sm flex-1 rounded-xl border border-slate-200 bg-white p-4 shadow-sm">
            <div className="flex items-start justify-between gap-3">
              ...
              <span
                className={`rounded-full px-2 py-1 text-xs font-semibold ${
                  session.status === 'Completed' ? 'bg-emerald-100 text-emerald-700' : 'bg-blue-100 text-blue-700'
                }`}
              >
                {session.status}
              </span>
            ...
            <button className="mt-4 w-full rounded-lg border border-yellow-300 bg-yellow-50 px-3 py-2 text-sm font-semibold text-yellow-700 hover:bg-yellow-100">
              Get Subscription
            </button>
          </article>
        ))}
